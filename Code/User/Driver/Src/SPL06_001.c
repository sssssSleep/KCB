...
//	I2C0_Engine(2,0,0);
	...
	uint8_t ch = val;
	HAL_I2C_Mem_Write(&hi2c1, hwadr<<1, regadr, I2C_MEMADD_SIZE_8BIT, &ch, 1,0xff);
...
//	reg_data = bI2C0_RxM_Data[0];
   HAL_I2C_Mem_Read(&hi2c1, hwadr<<1, regadr, I2C_MEMADD_SIZE_8BIT, &reg_data, 1,0xff);
...
//    spl0601_rateset(TEMPERATURE_SENSOR,32, 8);
    ...
    //Start background measurement
		...
		logBaroGroundPressureSum = spl0601_get_pressure()*10.0f;
		PID_Init(&spl_pid,SPL_Kp,SPL_Ki,SPL_Kd,SPL_INTEGRAL_MAX,SPL_INTEGRAL_MIN,SPL_OUTPUT_MAX,SPL_OUTPUT_MIN);
}
...
int16_t BaroAlt;
float baro_height;
uint8_t calibratingB = 20;
KalmanFilterData KFD_SPL={
	.Q = 0.01,
	.R = 1.3,
};
...
#endif
uint8_t SPL06_001_PID(float target_val)
{
	float res = 0.0f,val = 0.0f;
	//printf("spl: spl_data.baro_height = %f\r\n",spl_data.baro_height);
	res = PID_pos_realize(&spl_pid,target_val,spl_data.baro_height);
	val = res-target_val;
	//printf("spl: res = %f  val = %x\r\n",res,val>=0 ? (uint8_t)((int)val) : (uint8_t)((int)(-val)));
	if(val>=0)
	{
		if((int)val > 0x7F)
			return 0xFF;
		return 0x80 + (uint8_t)((int)val);
	}
	else if(val<0)
	{
		val = -val;
